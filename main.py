...
def setup_teams():
    ...
    nfl_teams = {
        "Arizona Cardinals": {"league": "NFC", "division": "NFC West"},
        "Atlanta Falcons": {"league": "NFC", "division": "NFC South"},
        "Baltimore Ravens": {"league": "AFC", "division": "AFC North"},
        "Buffalo Bills": {"league": "AFC", "division": "AFC East"},
        "Carolina Panthers": {"league": "NFC", "division": "NFC South"},
        "Chicago Bears": {"league": "NFC", "division": "NFC North"},
        "Cincinnati Bengals": {"league": "AFC", "division": "AFC North"},
        "Cleveland Browns": {"league": "AFC", "division": "AFC North"},
        "Dallas Cowboys": {"league": "NFC", "division": "NFC East"},
        "Denver Broncos": {"league": "AFC", "division": "AFC West"},
        "Detroit Lions": {"league": "NFC", "division": "NFC North"},
        "Green Bay Packers": {"league": "NFC", "division": "NFC North"},
        "Houston Texans": {"league": "AFC", "division": "AFC South"},
        "Indianapolis Colts": {"league": "AFC", "division": "AFC South"},
        "Jacksonville Jaguars": {"league": "AFC", "division": "AFC South"},
        "Kansas City Chiefs": {"league": "AFC", "division": "AFC West"},
        "Las Vegas Raiders": {"league": "AFC", "division": "AFC West"},
        "Los Angeles Chargers": {"league": "AFC", "division": "AFC West"},
        "Los Angeles Rams": {"league": "NFC", "division": "NFC West"},
        "Miami Dolphins": {"league": "AFC", "division": "AFC East"},
        "Minnesota Vikings": {"league": "NFC", "division": "NFC North"},
        "New England Patriots": {"league": "AFC", "division": "AFC East"},
        "New Orleans Saints": {"league": "NFC", "division": "NFC South"},
        "New York Giants": {"league": "NFC", "division": "NFC East"},
        "New York Jets": {"league": "AFC", "division": "AFC East"},
        "Philadelphia Eagles": {"league": "NFC", "division": "NFC East"},
        "Pittsburgh Steelers": {"league": "AFC", "division": "AFC North"},
        "San Francisco 49ers": {"league": "NFC", "division": "NFC West"},
        "Seattle Seahawks": {"league": "NFC", "division": "NFC West"},
        "Tampa Bay Buccaneers": {"league": "NFC", "division": "NFC South"},
        "Tennessee Titans": {"league": "AFC", "division": "AFC South"},
        "Washington Commanders": {"league": "NFC", "division": "NFC East"},
    }
...
def socialPost(id):
    ...
    ax.text(800, 0, f"elozone.pages.dev", fontsize=20, color=text, ha='center', va='center',fontproperties=font_prop)
...
def calc_order():
    ...
    afc_teams = {k: v for k, v in teams.items() if v["league"] == "AFC"}
    nfc_teams = {k: v for k, v in teams.items() if v["league"] == "NFC"}

    afcEast = {k: v for k, v in teams.items() if v["division"] == "AFC East"}
    afcWest = {k: v for k, v in teams.items() if v["division"] == "AFC West"}
    afcNorth = {k: v for k, v in teams.items() if v["division"] == "AFC North"}
    afcSouth = {k: v for k, v in teams.items() if v["division"] == "AFC South"}

    nfcEast = {k: v for k, v in teams.items() if v["division"] == "NFC East"}
    nfcWest = {k: v for k, v in teams.items() if v["division"] == "NFC West"}
    nfcNorth = {k: v for k, v in teams.items() if v["division"] == "NFC North"}
    nfcSouth = {k: v for k, v in teams.items() if v["division"] == "NFC South"}

    
    order['all'] = dict(sorted(teams.items(), key=lambda x: x[1]['elo'], reverse=True))

    order['AFC'] = dict(sorted(afc_teams.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['NFC'] = dict(sorted(nfc_teams.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['NFC East'] = dict(sorted(nfcEast.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['NFC West'] = dict(sorted(nfcWest.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['NFC South'] = dict(sorted(nfcSouth.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['NFC North'] = dict(sorted(nfcNorth.items(), key=lambda x: x[1]['elo'], reverse=True))

    order['AFCEast'] = dict(sorted(afcEast.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['AFCWest'] = dict(sorted(afcWest.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['AFCSouth'] = dict(sorted(afcSouth.items(), key=lambda x: x[1]['elo'], reverse=True))
    order['AFCNorth'] = dict(sorted(afcNorth.items(), key=lambda x: x[1]['elo'], reverse=True))
